...
const ImgStyled = styled('div')(({ theme, palette }) => ({
  position: 'relative',
  display: 'flex',
  alignItems: 'center',
...
const TestBar = styled('div')(({ theme }) => ({
  position: 'absolute',
  right: 0,
  top: 0,
  width: 20,
...
const ImagePlaceholder = styled('div')(({ theme }) => ({
  ...
  '&:hover': {
    backgroundColor: alpha(theme.palette.primary.second, 0.22)
  }
}));
...
const BannerDialog = ({ profile, setProfile }) => {
  ...
  const handleArweaveUpload = async () => {
    ...
    try {
      ...
      setProfile({ ...profile, banner: `https://arweave.net/${uploadRes?.data.id}` });
      setOpen(false);
  ...
  return (
    <>
      <ImgStyled palette={profile?.palette} onClick={() => setOpen(true)}>
        {profile.banner !== '' && (
          <img src={profile.banner} style={{ width: '100%', height: '100%', borderRadius: 14 }} />
        )}
        {profile.banner !== '' && <EditIcon palette={profile?.palette} />}
        {profile.banner === '' && (
          <>
            <LabelIcon
              sx={{
      ...
      <SimpleDialog
        ...
        <Typography
          ...
          Enter the URL of your desired image. Recommended image ratio is 3 by 1 (1200px by 400px).
        </Typography>
        <Row sx={{ alignItems: 'center' }}>
          <InputStyled
            palette={profile?.palette}
            id="url"
            placeholder="Enter the full banner URL here"
        ...
        <Typography
          ...
          {!uploadCost
            ? `Store your banner photo in a decentralized way. Recommended image ratio is 3 by 1 (1200px
                by 400px).`
